...
public class AirlineReservation {
    ...
    private static void initPassengers(String fileName) throws
        ...
        String[] header = a.nextLine().split(CSV_DELIMITER);
        planeRows = Integer.parseInt(header[0].trim());
        firstClassRows = Integer.parseInt(header[1].trim());
        businessClassRows = Integer.parseInt(header[2].trim());
        passengers = new String[planeRows];

        while (a.hasNextLine()) {
            String line = a.nextLine().trim();
            if (line.isEmpty()) continue;
            String[] parts = line.split(CSV_DELIMITER, 3);
            int row = Integer.parseInt(parts[0].trim());
            String name = parts[1].trim();
            if (!name.isEmpty()) passengers[row] = name;
        }
        a.close();
    }
    ...
    int c = findFirstRow(travelClass), d = findLastRow(travelClass);
        if (c == -1)
        {
            return false;
        }
    ...
    public static boolean book(int row, String passengerName) {
       if (passengerName == null) 
       {
        ...
        int c = findFirstRow(travelClass);
        int d = findLastRow(travelClass);
        for (int i = c; i <= d; i++) {
    ...
    public static boolean upgrade(String passengerName, int upgradeClass) {
        ...
        int c = findFirstRow(upgradeClass);
        int d = findLastRow(upgradeClass);
        int targetRow = -1;
        for (int i = c; i <= d; i++) {
            if (passengers[i] == null) {
                targetRow = i;
        ...
        passengers[targetRow] = passengerName;
